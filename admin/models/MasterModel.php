<?php
	...
	class MasterModel{
		...
		public function edit_news($edit){
			...
			$query = "UPDATE news SET title= '{$edit['txtName']}',  content = '{$edit['txtContent']}',description = '{$edit['description']}', images = '{$edit['image']}'   WHERE id = {$edit['id']} ";
			$results = mysqli_query( Database::$dbc, $query );
		...
		public function post_news($news){
			...
			$query = "INSERT INTO news( id,title,content,description,createdate,images,author,tags) VALUES ( '', '{$news['txtName']}', '{$news['txtContent']}','{$edit['description']}','{$news['date']}' , '{$news['image']}', '{$news['author']}', 1) ";
			$results = mysqli_query( Database::$dbc, $query );
